...
// Hàm tải danh sách sân bay cho dropdown
...
// Hàm khởi tạo danh sách chuyến bay với phân trang, lọc, tìm kiếm, sắp xếp
(function($) {
    function initFlightList(options) {
        ...
        window.loadFlights = function(page) {
            ...
            if (status !== '' && !['Scheduled', 'Delayed', 'Cancelled', 'Hidden'].includes(status)) {
...
function confirmDeleteFlight() {
    ...
    .then(data => {
        if (data.confirm_required) {
            ...
            $("#confirmDeleteFlightModal .modal-body").text(data.message);
            $("#confirmDeleteFlightBtn").off('click').click(function() {
                // Gửi yêu cầu xóa thật sự
                fetch("/banvemaybay/Admin/quantri/layout/include/handle_flight.php", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ action: "delete_confirmed", id: data.flight_id })
                })
                .then(response => response.json())
                .then(data => {
...
function addFlight(event) {
    ...
    let formData = {
        ...
        total_seat: $("#seat").val(),
        baggage_portable : $("#baggage_portable").val(),
        plane_flight : $("#plane_flight").val()
    };
...
// Hàm lưu chỉnh sửa chuyến bay sau khi nhập form
function updateFlight() {
    ...
    const formData = {
        ...
        baggage_portable : $("#edit_available_seats").val(),
        plane_flight : $("#edit_plane_flight").val()
    };
...
// Hàm mở form chỉnh sửa chuyến bay
    ...
    fetch(`/banvemaybay/Admin/quantri/layout/include/handle_flight.php?action=get&id=${id}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                $("#edit_available_seats").val(flight.baggage_portable);
                $("#edit_plane_flight").val(flight.plane_flight);
        ...
        .catch(error => {
            ...
            showNotification("Đã xảy ra lỗi khi gọi API chuyến bay.", "error");
        });
}
