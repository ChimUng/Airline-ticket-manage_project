...
function getStatusBadgeClass($status) {
    $statusClasses = [
        'Success' => 'bg-success text-white',       // Màu xanh lá + chữ trắng
        'Fail'    => 'bg-danger text-white',        // Màu đỏ + chữ trắng
    ];
    return $statusClasses[$status] ?? 'bg-secondary text-white'; // Mặc định: xám + chữ trắng
}
...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    try {
        ...
        $response['debug_query'] = "SELECT COUNT(*) as total FROM bookings b JOIN users u ON b.user_id = u.user_id     
        JOIN booking_flight bf on bf.booking_id = b.booking_id
        JOIN flight f ON bf.flight_id = f.flight_id 
        ...
        $response['debug_query_main'] = "SELECT b.*, u.username, f.flight_number, f.departure_date, f.departure_time, b.status_bookings as payment_status_payment FROM bookings b 
        JOIN users u ON b.user_id = u.user_id     
        JOIN booking_flight bf on bf.booking_id = b.booking_id
        JOIN flight f ON bf.flight_id = f.flight_id
        LEFT JOIN payment p ON b.payment_id = p.payment_id 
        WHERE 1=1 $statusCondition $searchCondition $orderBy 
        LIMIT $itemsPerPage OFFSET $offset";

        // Lấy dữ liệu booking cho trang hiện tại
        $query = "
            SELECT b.*, u.username, f.flight_number, f.departure_date, f.departure_time,
                    b.status_bookings as payment_status_payment
            FROM bookings b
            JOIN users u ON b.user_id = u.user_id
            JOIN booking_flight bf on bf.booking_id = b.booking_id
            JOIN flight f ON bf.flight_id = f.flight_id
