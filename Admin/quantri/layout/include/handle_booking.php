...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'get_details') {
        ...
        $stmt = $conn->prepare("
            SELECT b.*, u.username, u.email, u.phonenumber,
                    ...
                    pm.method_name, b.booking_date, b.status_bookings
            FROM bookings b
            ...
            JOIN payment_methods pm ON p.payment_methods_id = pm.payment_methods_id 
            WHERE b.booking_id = ?
        ");
        ...
        $seats = $conn->query("
            SELECT s.* 
            FROM seats s
            JOIN passengers p ON s.seat_id = p.seat_id
            JOIN booking_flight bf on bf.booking_id = p.booking_id
            WHERE bf.booking_id = '$bookingId'
        ")->fetch_all(MYSQLI_ASSOC);
        ...
        if (!empty($passengerIds)) {
            $baggageList = $conn->query("
                SELECT baggage_add FROM passengers 
    ...
    elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $conn->prepare("
            SELECT b.status_bookings, b.payment_status, bf.flight_id 
            FROM bookings b
            JOIN booking_flight bf on b.booking_id = bf.booking_id
            WHERE b.booking_id = ?
        ");
            ...
            case 'cancel': // Người dùng hoặc admin hủy
                ...
                if ($currentStatus !== 'used') {
                    ...
                    if (!empty($seats)) {
                        ...
                        // Tăng available_seats trong flight
