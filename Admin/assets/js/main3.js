$(document).ready(function() {
    ...
    function renderBookingDetails(modal, data) {
        ...
        const determineDetailedStatus = (booking) => {
            ...
            const paymentText = booking.status_bookings === 'Success' ? 'Đã thanh toán' : 'Chưa thanh toán';

            if (booking.status_bookings === 'Success') return {
                text: 'Đã thanh toán',
                class: 'bg-secondary text-white',
                icon: 'fa-hourglass-half',
                paymentText: paymentText
            };
            if (booking.status_bookings === 'Fail') return {
                text: 'Đã hủy',
        ...
        data.passengers.forEach(passenger => {
            ...
            const baggageInfo = baggage ? 
                `${baggage.baggage_add}kg ký gửi` : 
                'Không có';
            html += `
                <tr>
                    <td>${escapeHtml(passenger.full_name)}</td>
                    <td style="color:white;">${getTicketTypeBadge(passenger.ticket_type)}</td>
                    <td>${escapeHtml(passenger.baggage_add)}</td>
                </tr>`;
        });
